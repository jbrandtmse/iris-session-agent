...
- **Prepare-failure path returns bare success-shaped envelope without `isError:1`.**
  ...
  - **Owner:** **Deferred to v2 / Epic 11+ — not blocking v1.0.0 acceptance per Story 10.9 carry-forward triage (2026-05-07).** **Fourth Rule 9 recovery threshold reached** — Story 10.9 was the bound named successor and the empirical-validation scope of Story 10.9 (Devin Journey 2 walkthrough + NFR-P6 cache verification + FR59 cross-matrix gate) does not fit a 9-tool defensive sweep across the Inspection family. Per the Story 9.0 AC-6 compounding-recovery clause: *"If Story 10.9 also can't carry, the recovery note compounds (fourth-recovery binding required — at which point a focused dedicated cleanup story should be opened rather than further re-binding)."* **Action: open a dedicated v2 cleanup story** (proposed name: *"Inspection-tool defensive `EnsureIsErrorOnPrepareFailure` sweep across the 9-tool family"*) rather than re-binding to a fifth named carrier. Full drift history: Story 4.5 → Story 4.7 → Story 8.0 → Story 9.0 → Story 10.9 → v2 dedicated story. Original ownership reasoning preserved below for the v2 spec author. Original Owner: Story 10.9 (PRD v1 Completion Validation Walkthrough) — bound by Story 9.0 / Epic 8 retro per Rule 9 (named-successor-binding); originally bound to Story 4.7 → silently dropped → Story 7.0 re-bound to Story 8.0 → Story 8.0 re-bound to Story 9.0 → Story 9.0 re-bound to Story 10.9 → Story 10.9 escalates to dedicated v2 story.
