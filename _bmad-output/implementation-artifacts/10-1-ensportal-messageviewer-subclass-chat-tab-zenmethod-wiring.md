...
| 2026-05-07 | 1.2 | F-1 (search-agent tab-click render defect surfaced by Story 10.9 walkthrough) closed by **Story 10.10 hotfix** on 2026-05-07. TWO root causes empirically confirmed via `^ClineDebug` instrumentation + chrome-devtools-mcp browser session: (a) parent `EnsPortal.Template.filteredViewer:onTabChange()` `switch (currTab) { case 1..4; default: break; }` ignored 5th tab — fixed by overriding `onTabChange()` ClientMethod that calls `invokeSuper()` then `zen('chatPanelHost').refreshContents()` when `currTab===5`; (b) `%session.Data("SessionAgentMessageSearchKey")` bare-subscript read raised `<UNDEFINED>` on first-time render — fixed by wrapping in `$Get(...)`. Both fixes in `src/SessionAgent/EnsPortal/MessageViewer.cls` only (minimum blast radius). 2 new tests added to `MessageViewerTest` (`TestOnTabChangeOverridePresent`, `TestDrawChatPanelGuardsSessionDataAccess`). All 4 LLM providers re-walkthrough'd via `SessionAgent.UI.AgentConfig` Zen form (NOT direct SQL UPDATE — per `feedback_use_config_screens_not_sql.md`); chat panel rendered cleanly on tab-click for all 4. Regression sweep 417/417/0 (was 415, +2 new). See [`deferred-work.md`](deferred-work.md) §"F-1 — RESOLVED in Story 10.10". | Dev (Opus 4.7 1M) |
