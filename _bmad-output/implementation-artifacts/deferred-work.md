...
- **The Story 3.5 AC-6 system-prompt language requiring the LLM to disclose cross-session reaches in its response is unreliable when conversation context has many same-session prior turns. Programmatic enforcement is needed.**
  ...
  - **Owner:** Future story TBD — likely an Epic 4 story (when the full 13-tool catalog ships and cross-session reach becomes more common during operator triage of related sessions) OR a dedicated security/UX hardening story between Epic 3 and Epic 4. Per Rule 9, the spec author for the carrier story MUST grep `deferred-work.md` for "Story 3.7 lead-driven walkthrough" and incorporate this entry into the ACs.
