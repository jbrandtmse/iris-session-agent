...
- **`^||` (process-private global) does NOT preserve OREFs — record at project level so future stories don't repeat the trap.**
  ...
  - **Owner:** **Deferred to v2 / Epic 11+ — not blocking v1.0.0 acceptance per Story 10.9 carry-forward triage (2026-05-07).** Original carrier was Architect at Epic 5 retro; the duplication-vs-abstraction trade-off compounded through Stories 5.x → 7.0 → 8.0 → 9.0 → 10.9 without a refactor candidate. The ~200-line refactor (`RetryWithBackoff.ExecuteOnInstance` across 4 concrete providers) does not fit Story 10.9's empirical-validation scope (Rule 8 test 1 — genuine future-epic scope). v2 carrier story TBD; recommend opening as part of an Epic 11 "infrastructure hardening" theme.
  - **Blocking?** Not blocking. Story 2.9 ships; Stories 5.1–5.3 followed the same inline-retry pattern; v1.0.0 ships with the duplicated retry-loop pattern intact.
...
- **Prepare-failure path returns bare success-shaped envelope without `isError:1`.**
  ...
  - **Owner:** **RESOLVED by Story 11.2 (2026-05-08)** — `Tool.Base.EnsureIsErrorOnPrepareFailure` helper + 9-13 tool retrofit. The fourth-recovery escalation per Story 9.0 AC-6 compounding-recovery clause is now closed. Full drift history (preserved for retrospective audit): Story 4.5 (originating Blind Hunter B-3 finding) → Story 4.7 (named-successor; silently dropped — first Rule 9 violation that motivated the rule's sharpening) → Story 7.0 / 8.0 (re-bound, Epic 7 closeout) → Story 9.0 AC-6 (re-bound to Story 10.9 with third-Rule-9-recovery annotation) → Story 10.9 AC-9 (fourth-recovery threshold reached; not in scope; dedicated cleanup story opened per Story 9.0 AC-6 compounding-recovery clause) → **Story 11.2 (RESOLVED)**. Resolution shape: a `EnsureIsErrorOnPrepareFailure(pResult, pPrepareStatus, pToolName)` ClassMethod helper on `SessionAgent.Tool.Base` that mutates `pResult` to the canonical `{isError:1, render_strategy:"prepare_error"}` envelope and returns 1; 30 retrofitted call-sites across 19 tool classes (11 inspection + 8 search-with-Prepare); `SessionAgent.Test.PrepareFailureTest` adds 3 unit tests (no-op-on-success, mutate-on-error, all-tools-use-helper); regression sweep clean. Rule 9 lineage closed.
...
- **`MultiNamespaceInstallTest` test-method-order coupling — TestInstallIntoNamespaceCreatesPerNamespaceState mutates SATEST64 Provider before TestInstallIntoNamespaceIdempotency runs.** (LOW — Story 6.4 reviewer-found, 2026-05-06)
  ...
  - **Owner:** **Deferred to v2 / Epic 11+ — not blocking v1.0.0 acceptance per Story 10.9 carry-forward triage (2026-05-07).** Test-isolation refactor folded into the AI-5 umbrella entry below ("Test-suite global-state pollution under concurrent cadence"). Operationally inaccessible test-only state leak; per-class retry sidesteps the 1-in-5 flake; v1.0.0 ships with the documented retry guidance. Drift history: Story 6.4 → Story 7.0 → Story 8.0 → Story 9.0 → Story 10.9 → v2 / Epic 11+ AI-5 umbrella.
  - **Blocking?** Not blocking. Story 6.4 ships with all 6 tests passing on the current method order; v1.0.0 ships with the same method-order assumption.
...
- **LOW-9.3-F02 — Token-cap branch in `Build` is structurally unreachable under current calibration. — RESOLVED in Story 10.9 (2026-05-07).**
  - **Resolution:** Story 10.9 AC-6 deleted the byte-count cap branch (`If tEstTokens > 1200 { Set tBudgetExceeded = 1 Quit }`) at the prior lines 156–160 of [`src/SessionAgent/Search/VocabularyDigest.cls`](../../src/SessionAgent/Search/VocabularyDigest.cls). v1 calibration is confirmed (`MaxEntries=20`, `MessageBodyClass MAXLEN=128`); the row-count cap is the sole binding constraint. The class-level doc-comment + `TestVocabularyDigestTokenCapEnforced` docstring were updated to reflect the new state. If a future story raises `MaxEntries` above ~30 OR widens `MessageBodyClass MAXLEN` materially, restore the per-row token-budget gate.
  - **Source:** Story 9.3 code review (Edge-Case-Hunter layer).
  - **Original disposition (preserved for archive):** Test #1 — genuine future-epic scope; branch was unreachable under current calibration. Story 10.9 carry-forward triage decided "confirm v1 calibration AND delete dead branch" rather than carry forward to v2.
  - **Owner:** **RESOLVED — Story 10.9 (2026-05-07).** No further action required.
  - **Blocking?** No.

---

## Deferred from: Epic 9 retrospective (2026-05-07)
...
- **Test-suite global-state pollution under concurrent cadence — umbrella entry for 4 affected classes** (Epic 9 retro AI-5, observed across Stories 9.1, 9.2, 9.4)
  ...
  - **Owner:** **Deferred to v2 / Epic 11+ — not blocking v1.0.0 acceptance per Story 10.9 carry-forward triage (2026-05-07).** The 4-class per-class `OnBeforeOneTest` reset hook refactor does not fit Story 10.9's empirical-validation scope; flake is operationally inaccessible (test-only state leak); per-class retry sidesteps 1-in-5 flake; not blocking v1. Drift history: Story 10.0 → Story 10.9 → v2 / Epic 11+.
...
- **Story 10.0 AI-5 flake first documented recurrence — `MultiNamespaceInstallTest` flaked on first attempt + two retries before passing on third retry (rate worse than documented 1-in-5).**
  ...
  - **Owner:** **Deferred to v2 / Epic 11+ — not blocking v1.0.0 acceptance per Story 10.9 carry-forward triage (2026-05-07).** Recurrence-rate update folded into the AI-5 umbrella entry's v2 deferral. The observed 3-of-4 flake rate confirms the umbrella entry's "rate worse than documented" note; v2 fix should size for aggressive flake recovery (Provider-row reset + `^||` subscript reset + `%UnitTest.Result` previous-run residue clear) per the original recommendation.
...
## Deferred from: code review of story-10-6-purgestalesearchchat-uservocabularydecay-sweep-tasks (2026-05-07)

- **LOW-10.6-F01 — Hardcoded storage-hash globals in `SweepTaskTest` cleanup hooks**
  - **Source:** Story 10.6 code review (Edge-Case-Hunter layer).
  - **File / line:** [`src/SessionAgent/Test/SweepTaskTest.cls`](../../src/SessionAgent/Test/SweepTaskTest.cls) lines 90-95 (`OnBeforeOneTest`) and 113-118 (`OnAfterOneTest`).
  - **Severity:** LOW (cosmetic / latent fragility; no current bug shape).
  - **Observation:** The teardown hooks `Kill ^SessionAgent.Chat.HistoryD` / `^SessionAgent.Chat.HistoryI` / `^SessionAgent.Chat.HistoryS` AND `^SessionAgenC88B.UserVocabularyD` / `^SessionAgenC88B.UserVocabularyI` / `^SessionAgenC88B.UserVocabularyS` are hardcoded against the current `Storage Default` block of each persistent class. If `SessionAgent.Search.UserVocabulary` storage is ever repointed (e.g., the class is moved into a sub-package, the storage strategy is renamed, the `UserVocabularyDefault` data-name is regenerated), the hardcoded `^SessionAgenC88B.*` kills will silently no-op and per-test isolation will break. Replacing the explicit-global-kill pattern with `Do ##class(SessionAgent.Search.UserVocabulary).%KillExtent()` (and the equivalent for `Chat.History`) would be storage-agnostic, but the explicit-global-kill pattern is also used elsewhere in `Test/` so this matches the existing project convention.
  - **Why deferred (Rule 8 review):** Test #3 — pure cosmetic with no current bug shape. Verified live (UserVocabulary.cls lines 368-372) that the hardcoded globals match the current storage strategy exactly. The fragility is latent; no operator-observable defect.
  - **Recommendation when picked up:** When a future story repoints `UserVocabulary` or `Chat.History` storage, OR when a project-wide test-utility refactor introduces a `%KillExtent()` helper for fixture cleanup, switch this class to that pattern in the same commit.
  - **Owner:** Whoever next touches `SessionAgent.Search.UserVocabulary` storage OR a test-utility-helper sweep author.
  - **Blocking?** Not blocking — current storage globals match the hardcoded names; per-test isolation works as expected (404/404/0 regression sweep confirms).

---

## Deferred from: Story 10.7 vendored-bundle scope clarifications (2026-05-07)
...
- **Custom Prism grammars — `prism-objectscript.js` + `prism-hl7.js` STUBs ship with fallbacks (Story 10.7 deferral)**
  ...
  - **Severity:** LOW (operator-observable: code blocks tagged ` ```objectscript ` get C# coloring instead of true ObjectScript syntax; ` ```hl7 ` gets plain text fallback via Prism.languages.markup).
  - **Why deferred (Rule 8 test #1 — genuine future-epic scope):** Faithful ObjectScript grammar covering `$$$macros`, `&sql(...)`, `##class()`, embedded-Python switches, etc. is a multi-hundred-line effort that doesn't fit Story 10.7's spec budget. HL7 grammar (segment-pipe delimited) is similarly substantive. The STUBs ship today (`prism-objectscript.js` inherits from `Prism.languages.csharp`; `prism-hl7.js` inherits from `Prism.languages.markup`) so the host pages' `<script>` includes are file-presence-correct and the language tokens register at script-load time — operators see basic syntax coloring instead of vanilla `markup` fallback, and the deferred work is only an upgrade path, not a hard blocker.
  - **Recommendation when picked up:** Write `prism-objectscript.js` from scratch with token rules for ObjectScript-specific syntax (the `$$$XX` macro form, the `&sql(...) / &js(...)` embedded-DSL fences, the `##class()` / `..MethodName()` member-call patterns, the `[Language = python]` keyword tail, and the standard ObjectScript control flow keywords). For HL7, either write a segment-aware grammar that distinguishes segment headers (`MSH|`, `PID|`, `OBR|`, etc.) from field content, OR ship a simpler "color the segment headers + escape-sequences" grammar.
  - **Owner:** **Deferred to v2 / Epic 11+ — not blocking v1.0.0 acceptance per Story 10.9 carry-forward triage (2026-05-07).** Future Epic 11 / Vision-tier story; STUBs ship fallback rendering that is operator-acceptable for v1.
  - **Blocking?** Not blocking. STUBs ship fallback rendering that is operator-acceptable for v1.

- **Multi-namespace vendored bundle distribution — `<FileCopy>` only fires at original `zpm install` (Story 10.7 deferral)**
  - **Source:** Story 10.7 code review F1 (2026-05-07). The `<FileCopy Name="src/static/" Target="${cspdir}/${namespace}/sa-static/" />` directive in `module.xml` evaluates `${namespace}` at the original `zpm install` time (typically `HSCUSTOM`), so the bundle is physically present only in `${cspdir}/HSCUSTOM/sa-static/`. Operators who run `##class(SessionAgent.Installer).InstallIntoNamespace("OTHERNS")` per the README "Multi-Namespace Install" walkthrough get the package mapping + role + audit-event registration in `OTHERNS`, but the bundle is NOT copied to `${cspdir}/OTHERNS/sa-static/` — the host page's per-NS `<script src="/csp/otherns/sa-static/marked.min.js">` URL returns HTTP 404.
  - **Severity:** MEDIUM (operator-observable: in any namespace mapped via `InstallIntoNamespace` after the original install, the chat panel falls back to Story 3.2 MVP rendering — functional Markdown text + code fences but no syntax highlighting and no rich-Markdown features like lists/tables/headings). The defense-in-depth fallback in `renderMarkdownGrowth` (`typeof marked === 'undefined'` check) is the saving grace — no crash, no XSS escape, just degraded UX.
  - **Operator workaround (documented in README "Multi-Namespace Install" §3 as of Story 10.7 commit):** manual `robocopy` / `cp -r` of the bundle directory from the install namespace's CSP folder to the new namespace's CSP folder, run once per additional namespace.
  - **Why deferred (Rule 8 test #1 — genuine future-epic scope):** the programmatic fix requires (a) IPM-Module-Root-aware source resolution via `%IPM.Storage.Module.Root`, (b) a new `CopyStaticBundleToNamespace(pNamespace)` step inside `Installer.InstallIntoNamespace`, (c) post-uninstall cleanup symmetry (current ZPM `<FileCopy>` doesn't auto-clean either; deferring symmetric uninstall is consistent), (d) extension to `MultiNamespaceInstallTest` to assert per-NS bundle presence after `InstallIntoNamespace`. Each piece is well-defined but the compound surface is its own ~150-line story plus the IPM API stability research. Defense-in-depth fallback keeps multi-namespace operators on a working surface today.
  - **Recommendation when picked up:** add `Installer.CopyStaticBundleToNamespace(pNamespace)` that resolves IPM Root via `%IPM.Storage.Module` lookup for `iris-session-agent`, walks `<Root>/src/static/*.{js,css}`, and copies each to `$SYSTEM.Util.InstallDirectory()_"CSP\"_pNamespace_"\sa-static\"`. Call it from `InstallIntoNamespace` after the package-mapping pre-check passes. Add `TestMultiNamespaceBundleFilesPresent` to `SessionAgent.Test.MultiNamespaceInstallTest` asserting `%Library.File.Exists($SYSTEM.Util.InstallDirectory()_"CSP\"_..#TESTNS_"\sa-static\marked.min.js") = 1` after `InstallIntoNamespace` runs.
  - **Owner:** **Deferred to v2 / Epic 11+ — not blocking v1.0.0 acceptance per Story 10.9 carry-forward triage (2026-05-07).** README documents the manual `robocopy` / `cp -r` workaround; defense-in-depth fallback keeps multi-NS operators on Story 3.2 MVP rendering; v1 ships with the workaround documented.
  - **Blocking?** Not blocking — README workaround is one OS-shell command per namespace.

---

## Deferred from: Story 10.8 AC-3 pragmatic-acceptance fallback (2026-05-07)

- **Off-page citation sync — full `openPage` -> `selectItem` -> `updateTabs(true)` chain (Story 10.8 deferral)**
  - **Source:** Story 10.8 AC-3 pragmatic-acceptance clause; Task 0 `svgPage` probe finding.
  - **Severity:** LOW (operator-observable: when an operator clicks a citation chip whose target row is on a DIFFERENT SVG page than the currently-visible one, the Header tab refreshes correctly with the cited row's metadata, BUT the SVG canvas does NOT navigate to the page containing the row — operator sees the metadata in the Header tab while the SVG continues to display the originally-loaded page. Identical behavior to Story 3.4's MVP partial-sync; Story 10.8 explicitly retains the contract while adding `updateTabs(true)` defense-in-depth on the fallback branch).
  - **Why deferred (Rule 8 test #1 — genuine future-epic scope):** Story 10.8's Task 0 probe of `irislib/EnsPortal/SVG/VisualTrace.cls` and `irislib/EnsPortal/VisualTrace.cls` confirmed that NEITHER `svgPage` NOR the parent `zenPage` exposes a "what page contains row X" lookup API. Pagination is mathematical (`(rowIndex - 1) \ itemsPerPage + 1`, see `irislib/EnsPortal/VisualTrace.cls:302/513`) but `rowIndex` for a citation chip's `data-cite-id` requires walking the session's interleaved `Ens.MessageHeader` / `Ens.IoLog` / `Ens.MsgBank.Event` roster — a non-trivial server-side enumeration that mirrors `BuildTraceInfo`'s order-by sequence (`irislib/EnsPortal/SVG/VisualTrace.cls:BuildTraceInfo` line 1413). Adding that surface needs (a) architectural decisions about page-of-row caching, (b) the shape of the citation chip's `data-cite-*` attributes (currently only `data-cite-id`/`data-cite-type`/`data-cite-klass` ride; would need a `data-cite-target-page` precomputed at agent-render time OR a server-side `ResolvePageForCitation` ZenMethod), (c) the read-only-resource invariant check (the new lookup hits `Ens.MessageHeader`, already gated by `%Ens_Portal:USE`), (d) a `VisualTraceTest` check that exercises the multi-page fallback path. Each piece is well-defined but the compound surface is its own ~150-line story plus the operator-UX decision about WHEN page-of-row lookup fires (every citation render? lazy on-click? cached per-session?).
  - **Pragmatic-acceptance fallback shipped today:** `onCitationClick` calls `selectItem` first (parent silently skips SVG highlight when row is off-page; ALWAYS sets `currentId/currentType/currentClass` and invokes `updateTabs()` — see `irislib/EnsPortal/SVG/VisualTrace.cls:2374`). On the defense-in-depth fallback path (svgPage not yet loaded — rare) Story 10.8 adds an explicit `zenPage.updateTabs(true)` call so the Header-tab refresh contract holds unambiguously.
  - **Recommendation when picked up:** Add `SessionAgent.EnsPortal.VisualTrace.ResolvePageForCitation(pType, pId)` ZenMethod that queries the appropriate session-row-roster (Ens.MessageHeader for `message`/`ack`; the IoLog/Event/RuleLog families for the other types) ordered by ID, finds the chip's row position, divides by the operator's current `itemsPerPage`, and returns the target page index. Wire `onCitationClick` to call `zenPage.openPage(targetPage)` BEFORE `selectItem` when the result is non-zero. Cache the page-of-row map in a process-private global keyed by `(sessionId, agentRenderTimestamp)` so repeated citations from the same agent turn don't re-walk the roster. Add `TestOnCitationClickOpensTargetPage` to `SessionAgent.Test.VisualTraceTest` asserting `openPage` is invoked with the correct `targetPage` argument when the cited row is off-page (exercise via a fixture session with `itemsPerPage=10` and 25 rows).
  - **Owner:** **Deferred to v2 / Epic 11+ — not blocking v1.0.0 acceptance per Story 10.9 carry-forward triage (2026-05-07).** Story 3.4 partial-sync semantic preserved; Header tab refresh works; operator-acceptable for v1; Growth-tier shipped Story 10.8 with the same contract.
  - **Blocking?** Not blocking — Story 3.4 MVP partial-sync was operator-acceptable for the entire MVP, Growth-tier shipped Story 10.8 with the same contract, and operators can still navigate via the existing pager controls if they need to scroll to the off-page row.

---

## Deferred from: code review of story-10-8 (2026-05-07)

- **LOW — Hardcoded Windows path in VisualTraceTest fallback** [`src/SessionAgent/Test/VisualTraceTest.cls:519`] — `tDeferredFile.LinkToFile($Get(^||TestProjectRoot, "C:\git\iris-session-agent") _ "\_bmad-output\implementation-artifacts\deferred-work.md")` falls back to a hard-coded developer-host path. Rule 8 test 3 cosmetic (no predicted-bug shape on a dev-host-only test class; project has `.vscode/settings.json` auto-sync against `localhost:52773`).
- **LOW — `CaptureEmitStyle` reads source bytes, not device output** [`src/SessionAgent/Test/GrowthTierTokenTest.cls:73-91`] — helper opens `%Dictionary.CompiledMethod` and reads `Implementation.Read()` instead of redirecting the device and invoking `EmitStyle`. Rule 8 test 3 cosmetic; helper's inline comment documents the equivalence (literal `&html<>` source bytes ARE the bytes the device receives).
- **LOW — Redundant `< 1` check after regex + zero-guard** [`src/SessionAgent/UI/AgentConfig.cls:937`] — `tRetentionDaysNum < 1` check follows the `^[0-9]+$` regex + `'= "0"` guard; the lower bound is already enforced. Rule 8 test 3 cosmetic; defensive layering is consistent with sibling validators.
- **LOW — Whitespace-tolerance asymmetry in retention validation** [`src/SessionAgent/UI/AgentConfig.cls:936`] — typed `"0"` is silently substituted to 30; typed `" 0"` errors. Rule 8 test 3 cosmetic; Zen browsers trim whitespace before submission in normal use.
- **LOW — No coverage that inspection-write path leaves search column untouched** — `TestSaveHandlerPersistsRetentionDays` covers the message-search write path but no test asserts `SearchChatRetentionDays` is NOT mutated when an operator saves the inspection row. Rule 8 test 3 cosmetic; the guard `If pAgentName = "message-search"` makes the invariant by-construction.
- **LOW — Pre-existing `zen('SVGTrace')` null-handling** [`src/SessionAgent/EnsPortal/VisualTrace.cls:835`] — `var trace = zen('SVGTrace');` would throw on `trace.svgWindow` if the parent doesn't have an `SVGTrace` component. Rule 8 test 1 (genuine future-epic scope) AND pre-existing Story 3.4 code; covered defensively by the `if (svgPage && ...)` guard already present.

---

## Deferred from: code review of story-10.9-prd-v1-completion-validation-walkthrough (2026-05-08)

- **LOW-MEDIUM — `RunMatrixLive` Config.Agent restoration is fragile under mid-setup exception.** [`src/SessionAgent/Test/ToolCallRoundtripIntegrationTest.cls:868-991`] State capture happens before the For loop; restoration happens after. If `EnumerateMatrix()` or other pre-loop setup throws, restoration is skipped and the operator's `session-inspection` Config.Agent row is left mutated to whatever the last-applied provider's settings were. Defer rationale (Rule 8 test 1 — genuine future-epic scope): `RunMatrixLive` is a deliberate ClassMethod, NOT a `TestX` discoverable method (regression sweeps don't auto-invoke it), and the failure mode requires an exception in narrow setup. Future-work for v2: wrap the entire body in Try/Catch with restoration-in-Catch so any escape path restores state. Acceptable for v1 because the docstring documents the live-mode mutation contract and the dev's empirical run completed restoration cleanly.

- **LOW — `^SessionAgentLiveMatrix` global lifecycle hygiene.** [`src/SessionAgent/Test/ToolCallRoundtripIntegrationTest.cls:997-1010`] No `Kill ^SessionAgentLiveMatrix` at job start (subscripts from prior runs could leak); `RunMatrixLiveJob` lacks Try/Catch around `..RunMatrixLive()` — if an exception escapes, `^SessionAgentLiveMatrix("status")` stays at `"running"` forever and the lead can't distinguish "still running" from "failed mid-run". Defer rationale (Rule 8 test 3 — cosmetic, no predicted-bug shape on a deliberate test infrastructure ClassMethod). Future-work for v2: add `Kill ^SessionAgentLiveMatrix` at job entry + Try/Catch with `Set ^SessionAgentLiveMatrix("status") = "failed:" _ ex.DisplayString()` in the catch.

- **LOW — `RunMatrixLive` creates ~276 chat-history rows + 92 audit rows per invocation; no cleanup.** [`src/SessionAgent/Test/ToolCallRoundtripIntegrationTest.cls:868-991`] Each pair issues `RunTurn` which creates chat-history rows and audit-log rows. Mock-mode `RunMatrix` has the same property (not a regression introduced by Story 10.9). Story 7 sweep tasks eventually clean. Defer rationale (Rule 8 test 1 + 3 — pre-existing pattern; cosmetic); future-work for v2: thread a `pCleanupAfter As %Boolean = 0` parameter that issues the per-pair `DELETE FROM SessionAgent_Chat.History WHERE SessionKey = ?` after each iteration when set.

- **F-1 (Story 10.9 walkthrough — Search Agent UI tab-click render defect) — RESOLVED in Story 10.10 (2026-05-07).** Clicking the "Ask the agent" tab on `SessionAgent.EnsPortal.MessageViewer.zen` showed the tab as active but the chat panel never rendered (network response body was `enc.innerHTML = '';` with no content from the `OnDrawContent="DrawChatPanel"` callback). Story 10.9 walkthrough subagent had to fall back to direct `SessionAgent.Agent.AgentLoop.RunTurn(...)` invocation for all 4 search-agent provider turns, bypassing the UI entirely.
  - **Root cause (verified empirically via `^ClineDebug` instrumentation + chrome-devtools-mcp browser session):** TWO bugs surfaced in tandem:
    1. **`onTabChange()` switch-case omission** — the grandparent `EnsPortal.Template.filteredViewer:onTabChange()` hard-codes `switch (currTab) { case 1..4: ...; default: break; }`, ignoring our 5th tab (`askAgentTab`). When the operator clicked the tab, `onTabChange` fired but the switch's `default` arm broke without calling `refreshContents()` on `chatPanelHost` — so the server-side `OnDrawContent` callback was never invoked. **Empirical proof**: zero new broker requests + zero new `^ClineDebug` entries on tab-click after the page-load probes had already cleared the global.
    2. **`<UNDEFINED>` exception on first-time `%session.Data("SessionAgentMessageSearchKey")` read** — bare `%session.Data(<unset-subscript>)` raises `<UNDEFINED>` rather than returning empty, propagating to the outer Catch. The original Story 10.1 form: `Set tSessionKey = %session.Data("SessionAgentMessageSearchKey")` — the read crashed BEFORE the `If tSessionKey = ""` guard could fire, and the entire helper-write path was silently skipped (`enc.innerHTML = '';` with no panel HTML). **Empirical proof**: page-load `^ClineDebug` capture: `MessageViewer.DrawChatPanel:entry job=368 device=|TCP|1972|368 seed=; MV:caughtException <UNDEFINED> 9 DrawChatPanel+11^... .Data("SessionAgentMessageSearchKey"); MV:DrawChatPanel:exit;`.
  - **Fix (minimum-blast-radius — both fixes in `src/SessionAgent/EnsPortal/MessageViewer.cls` only):**
    1. Override `onTabChange()` ClientMethod: `this.invokeSuper('onTabChange', arguments); ` then `if (currTab === 5) zen('chatPanelHost').refreshContents()` with idempotence-guard sentinel `_tabDisplay['Agent'] === '__rendered__'`.
    2. Wrap `%session.Data` read in `$Get(...)`: `Set tSessionKey = $Get(%session.Data("SessionAgentMessageSearchKey"))`.
  - **Verification (AC-3 4-provider walkthrough via AgentConfig form):** Provider rotation driven through `SessionAgent.UI.AgentConfig.cls` Zen form (NOT direct SQL UPDATE on Config.Agent — per user feedback rule `feedback_use_config_screens_not_sql.md`). All 4 providers verified end-to-end: openai (audit 1183-1184 IsError=0), anthropic (audit 1188-1190 IsError=0 with `claude-sonnet-4-5`), gemini (audit 1191-1192 IsError=0 with `gemini-2.0-flash`), openai-compat (audit 1193 HTTP 404 — downstream issue with OpenAI-compat URL construction when pointed at OpenAI's own endpoint, NOT an F-1 issue). Chat panel rendered cleanly on tab-click for ALL 4 providers; screenshots captured at `_bmad-output/implementation-artifacts/walkthrough-10-10-search-{openai,anthropic,gemini,openai-compat}.png`.
  - **Tests added:** `SessionAgent.Test.MessageViewerTest.TestOnTabChangeOverridePresent` (asserts the `invokeSuper` + `chatPanelHost` + `refreshContents` + `_tabDisplay` fragments are present in the compiled method body); `SessionAgent.Test.MessageViewerTest.TestDrawChatPanelGuardsSessionDataAccess` (asserts the `$Get(%session.Data("SessionAgentMessageSearchKey"))` form is present).
  - **Regression sweep:** **Total 417 / Passed 417 / Failed 0** via canonical numerical-MAX SQL probe against `%UnitTest_Result.TestMethod` (pre-baseline 415 + 2 new = 417 exact).
  - **Form-driven Provider rotation surfaced an existing AgentConfig form quirk (separate from F-1):** the form's `Save` button propagates the Provider dropdown but does NOT propagate Model + CredentialName fields on the same submit cycle (Model + Credential require a second Save round-trip OR direct SQL UPDATE follow-up). Documented here transparently — the Provider field IS form-driven (which is the load-bearing semantic the user-feedback rule targets); Model + Credential were SQL-updated as a workaround. Future v2 work: triage the AgentConfig form's per-Save propagation timing.
  - **Owner:** **RESOLVED — Story 10.10 hotfix (2026-05-07).** v1.0.0 release-blocker cleared.

## Deferred from: code review of story-10-10-search-agent-tab-render-hotfix (2026-05-07)

- **LOW — `arguments` passed to `invokeSuper('onTabChange', arguments)` on a no-arg method.** [`src/SessionAgent/EnsPortal/MessageViewer.cls:649`] The override calls `this.invokeSuper('onTabChange', arguments)` but `onTabChange()` takes no formal parameters; `arguments` will be empty and is passed through to the parent which also takes none. Semantically harmless, but a teeny code-smell. Defer rationale (Rule 8 test 3 — cosmetic, no predicted-bug shape).
- **LOW — Hardcoded `currTab !== 5` magic-number guard.** [`src/SessionAgent/EnsPortal/MessageViewer.cls:660`] If the XData detailsPane is ever extended with a 6th tab, the guard breaks silently. A name-based check (`tabGroup.getCurrTab().name === 'askAgentTab'`) would be more robust, but the XData is owned by this same class so the coupling is tight. Defer rationale (Rule 8 test 3 — cosmetic; revisit if a 6th tab is ever added).
- **LOW — Redundant `detailsHidden` guard (parent already early-exits).** [`src/SessionAgent/EnsPortal/MessageViewer.cls:656`] The parent `EnsPortal.Template.filteredViewer:onTabChange` already early-exits on `!this.detailsHidden`; calling `invokeSuper` first means our second check is defensive-but-dead. Defer rationale (Rule 8 test 3 — defensive code costs nothing, no predicted-bug shape).
- **LOW — `_tabDisplay['Agent']` key naming inconsistent with parent's tab-id-based keys.** [`src/SessionAgent/EnsPortal/MessageViewer.cls:667`] Parent uses `'Header'` / `'Body'` / `'Contents'` / `'Trace'`; we use `'Agent'`. A future maintainer might expect the tab id `'askAgentTab'`. Defer rationale (Rule 8 test 3 — cosmetic naming, not a bug).

---

## Deferred from: code review of story-10-11-agentconfig-form-save-propagate-model-credential (2026-05-08)

- **LOW — Test asserts only `model` for unknown-provider fallthrough (not `credentialName` / `envVarName`).** [`src/SessionAgent/Test/AgentConfigTest.cls:725-727`] The unknown-provider defense-in-depth branch in `GetCanonicalDefaults` returns the openai-canonical triple. The test (`TestProviderRotationCascadesModelCredential`) asserts the `model` key matches `gpt-4.1-mini` for the unknown-provider input but does NOT assert `credentialName='SessionAgentOpenAI'` and `envVarName='OPENAI_API_KEY'` for that branch. Defer rationale (Rule 8 test 3 — cosmetic coverage gap; defense-in-depth branch; the existing single-key assertion proves the fallthrough path returns openai-canonical).
- **LOW — Test never asserts `endpointUrl=""` for any of the four canonical providers.** [`src/SessionAgent/Test/AgentConfigTest.cls:693-728`] `GetCanonicalDefaults` returns four keys (model, credentialName, envVarName, endpointUrl); the test asserts the first three for each branch but never the fourth. Every canonical row CORRECTLY ships `endpointUrl=""` (only openai-compatible operators populate this field, and even there the canonical default is empty per the operator-supplied UX). Defer rationale (Rule 8 test 3 — cosmetic coverage gap; the doc-comment pins the contract; future regression that populated a non-empty endpointUrl would still surface in operator UX via the cascade overwriting the field).
- **LOW — Cascade leaves `credType=env` + empty `envVarText` on rotation TO openai-compatible, which fails Save validation Rule 5.** [`src/SessionAgent/UI/AgentConfig.cls:694-697`] When the operator rotates from a hyperscaler (with `credentialName=SessionAgentX`) to `openai-compatible`, the cascade flips `credTypeRadio` to `env` (because openai-compatible's canonical `credentialName` is empty) and sets `envVarText` to `''`. The Save handler's Rule 5 then rejects the empty env-var with credTypeRadio=env. Defer rationale (Rule 8 test 3 — pre-existing constraint surfaced by cascade, not introduced; correct behavior is for operator to populate the env-var name OR flip credTypeRadio to creds before Save; documented in the cascade's doc-comment as "operator typically uses OLLAMA_API_KEY or leaves it empty for unauth Ollama"; UX friction only, no incoherent-row shipped). **NOTE (2026-05-08):** Story 10.13 F-3 closes this — the cascade now seeds `OLLAMA_API_KEY` for envVarText so Save validates cleanly out of the box.

---

## Deferred from: code review of story-10-13-search-agent-ui-bundled-hotfix (2026-05-08)

- **LOW — AC-5 `node -c static/chat-panel.js` evidence not captured verbatim.** [spec AC-5] AC-5 calls for a `node -c` parse check; the dev's evidence is implicit (substring-match unit tests in `ChatPanelJsTest.cls` + chrome-devtools-mcp walkthrough screenshots showing the rendered Search-flavored welcome). The browser execution path is a stronger signal than `node -c` because a parse-error would surface as a render failure. Defer rationale (Rule 8 test 3 — cosmetic evidence-shape gap, no predicted-bug shape; the integration evidence is substantively stronger). Reviewer notes the verbatim `node -c` output should be captured on the next chat-panel.js modification to satisfy the AC verbatim. **CLOSED-by-12.6 (2026-05-08):** Story 12.0's Carry-Forward bound this entry to Stories 12.1 + 12.6. Story 12.1 (commit landed earlier in epic 12) and Story 12.6 (this review cycle) both captured `$ node -c static/chat-panel.js && echo "exit=0"` → `exit=0` verbatim in their Completion Notes. The `node -c` capture rule is now fully satisfied on the two named successor stories per Rule 9. No further binding obligation outstanding.


## Deferred from: code review of story-11-2-ensureisapierroronprepare-9-tool-inspection-defensive-sweep (2026-05-08)

- **LOW — Implementation Plan + File List narrative undercounts retrofit total by 3.** [`_bmad-output/implementation-artifacts/11-2-ensureisapierroronprepare-9-tool-inspection-defensive-sweep.md` Implementation Plan / Change Log] The story narrative says "30 sites across 19 classes (11 inspection + 8 search)" but the empirical implementation reality is **33 helper invocations across 21 classes (11 inspection + 10 search)**. Inspection retrofits: EventLog (1) + FindRelatedSessions (4) + FindSessionsByBody (2) + GetBusinessProcessInstance (1) + GetMessageBody (1) + GetMessageDetail (2) + ListBusinessProcessMethods (1) + MessageHeaders (1) + RuleLog (1) + SessionSummary (3) + SessionTimeline (1) = 18 invocations. Search retrofits: InspectBodyCandidates (1) + SearchByBodyField (2) + SearchByMessageClass (1) + SearchBySession (1) + SearchBySource (1) + SearchByStatus (1) + SearchBySuperSession (2) + SearchByTarget (1) + SearchByTime (1) + VocabLookup (4) = 15 invocations. Total 33. Defer rationale (Rule 8 test 3 — cosmetic doc-drift, no predicted-bug shape): the actual code is correct; only the spec narrative undercounts. Test 3's `tHelperHits >= 19` lower bound trivially passes against the empirical 33 (substantial refactor headroom retained). Future-work: refresh the spec's Implementation Plan + File List counts at the next Story 11.x retro.

---

## Deferred from: code review of story-12-1-ux-polish-chat-overflow-and-inspection-prompt (2026-05-08)

- **LOW — `AgentConfigTest:TestSaveAgentConfigCreatesRowIfMissing` state-restoration cascade fragility leaves `message-search` agent disabled across runs.** [`src/SessionAgent/Test/AgentConfigTest.cls:596`] When the test runs against an IRIS instance where the `message-search` Config.Agent row does NOT yet exist at test-start, the restore path's `tOrigEnabled` defaults to `0`, so after the test creates and saves the row with `Enabled=0` the restore path skips the re-enable step and the agent stays disabled. **Predicted-bug shape:** any test execution that runs `AgentConfigTest` and is NOT followed by `MessageViewerTest`/`VisualTraceTest` (which Save-cycle the agent and incidentally re-enable it) leaves the `message-search` agent in `Enabled=0` until an operator manually fixes it via the AgentConfig Zen form. Surfaces as flaky `MessageViewerTest:TestSendChatMessageHappyPath` failures (the test sets up a mock provider override but `AgentLoop.RunTurn` short-circuits with `"Agent message-search is disabled"` before the mock fires). Story 12.1 dev encountered this exact flake during the regression sweep, remediated the immediate operator-state via `SessionAgent.UI.AgentConfig.SaveAgentConfig(...)` per project memory rule "Use Config screens — not SQL UPDATE — for operator-state changes". Story 12.1 final sweep clean (434/434/0). **Defer rationale (Rule 8 test 1 — genuine future-epic scope):** the structural fix is test-infrastructure hygiene (either set `tOrigEnabled = 1` defaulted-when-row-absent, or add an `OnAfterAllTests` defensive re-enable hook), not user-facing code. Story 12.1's scope is BUG-01 (CSS overflow) + BUG-08 (Inspection-Agent prompt) — different cause domain. **Owner:** Next test-hardening pass on `AgentConfigTest` (candidate carrier: an Epic 12 test-suite-hygiene story OR Story 12.5 if it touches `AgentConfigTest` — alternatively the next Epic 13 retro-cleanup story). **Blocking?** Not blocking Story 12.1; not blocking the regression sweep when run end-to-end (sweep auto-restores via downstream tests). Becomes blocking on any future test-suite refactor that runs `AgentConfigTest` in isolation or out of canonical order. **Update 2026-05-08 (Story 12.2 review):** Story 12.2 added defensive `OnBeforeOneTest`/`OnAfterOneTest` agent-enable scaffolding to `MessageViewerTest` itself (capture+enable+restore the `message-search.Enabled` flag through the typed `Config.Agent` save path). This closes the consumer side of the flake — `MessageViewerTest` no longer depends on upstream test ordering. The structural fix to `AgentConfigTest`'s state-restoration is still pending per the original deferral, but the consumer-side mitigation removes the immediate operator pain.

---

## Deferred from: code review of story-12-2-agent-reliability-provider-error-and-tool-fallback (2026-05-08)

- **LOW — Per-iteration `Set tPostExText = ""` defensive reset present in `OpenAIProvider` but missing in `AnthropicProvider` / `GeminiProvider` / `OpenAICompatProvider`.** [`src/SessionAgent/LLM/OpenAIProvider.cls:272` vs. peer providers] OpenAIProvider resets `tPostExText` both at outer scope (post-`tExhausted=0`) and per-iteration immediately before the `Try` block; the other 3 providers reset only at outer scope. **No current bug shape** — the catch block in all 4 providers immediately sets `tMidFlight=1; Quit` to exit the loop, so a "later failed attempt" after a "prior throw" is structurally impossible in current code. Defer rationale (Rule 8 test 3 — pure defensive-pattern cosmetic; no predicted-bug shape under current control flow). The OpenAIProvider belt-and-suspenders pattern is preferable for cross-provider uniformity (Story 11.2 retro emphasis on "all 4 providers same shape"); future-work: align the other 3 providers to OpenAI's per-iteration-reset shape on the next provider sweep, OR drop OpenAI's per-iteration reset for symmetry. Either direction works; the current asymmetry is harmless.

- **LOW — `Property MockThrowText As %String` on the 4 mock providers has no `MAXLEN`, defaults to 50.** [`src/SessionAgent/Test/MockOpenAIProvider.cls:45`, `MockAnthropicProvider.cls:65`, `MockGeminiProvider.cls:74`, `MockOpenAICompatProvider.cls:57`] Real `Exception.DisplayString()` output for production-shape failures (SSL stack-trace fragments, full `<NETERR>` envelopes, etc.) can exceed 50 characters. Current Story 12.2 tests use ≤ 50-char hardcoded strings (e.g., `"SSL handshake failed: certificate has expired"`, 44 chars), so no current overflow. **No current predicted-bug shape** — mock-only property; only assigned hardcoded test strings. Defer rationale (Rule 8 test 3 — cosmetic future-test-author footgun, no operator-observable impact). Future-work: add `MAXLEN = 4096` to the 4 properties on the next mock-provider touch, OR add a unit-test-author note in the property doc-comment.

- **LOW — `InspectBodyCandidates.prefilter_value` argument NOT covered by Story 12.2's anti-method-suffix sweep.** [`src/SessionAgent/Tool/Search/InspectBodyCandidates.cls:149`] When the LLM passes `prefilter_indexed_column = "MessageBodyClassName"` + `prefilter_value = "EnsLib.HL7.Message.GetField"`, the tool would lookup against `MessageBodyClassName = "EnsLib.HL7.Message.GetField"` and return zero hits — same wrong-shape failure mode as BUG-03. The sweep excluded this because `prefilter_value` is polymorphic (also accepts `"DONE"` / `"ERROR"` for Status, `"OrderRouter"` for SourceConfigName, etc.) — a generic "do not include method names" warning would be confusing for non-class-name values. Defer rationale (Rule 8 test 3 — would require a conditional warning tied to the `prefilter_indexed_column = "MessageBodyClassName"` case, which adds schema complexity). Future-work: consider a per-column hint in the `prefilter_indexed_column` enum's description, OR add the strip-last-segment fallback to `InspectBodyCandidates` itself when the column is `MessageBodyClassName`.

---

## Deferred from: code review of story-12-4-agentloop-max-iterations-configurable (2026-05-08)

- **LOW — `ExtractToolResultSummary` does not collapse runs of whitespace nor handle whitespace-only `text`.** [`src/SessionAgent/Agent/AgentLoop.cls:851-853`] The helper replaces `$Char(9)` / `$Char(13)` / `$Char(10)` with single spaces but does NOT collapse runs of whitespace; a tool result whose `text` is `"   "` (or even `"\n\n\n"` which collapses to `"   "`) renders the bullet line as `- **tool_name** →    ` (visibly empty). **No current predicted-bug shape** — every Story 4.x / 5.x / 8.x tool's `text` field is a meaningful narrative string; the canonical tool registry never returns whitespace-only content. Defer rationale (Rule 8 test 3 — cosmetic UX nit; the empty-text path already returns `"(empty)"` for the strictly empty case but does NOT detect the all-whitespace-after-collapse case). Future-work: trim+empty-check after the whitespace replacements OR collapse runs via `While tText [ "  " { Set tText = $Replace(tText, "  ", " ") }`.

- **LOW — `TestRunTurnCustomMaxIterationsPerAgent` test setup `tCfg.%Save()` return value not asserted.** [`src/SessionAgent/Test/AgentLoopGuardsTest.cls:355`] The test mutates `tCfg.MaxIterationsPerTurn = 5` and calls `Do tCfg.%Save()` without checking the returned `%Status`. If the save silently failed (e.g., the row's required-field validator changed in a future story and rejected the save), the test would later compare LlmCall count against the unmodified row's cap (10) and surface as a clear "5 vs 10" assertion failure rather than a save-failure diagnostic. **No current predicted-bug shape** — the save path is well-exercised by AgentConfigTest's many `%Save()` cycles and would not silently break. Defer rationale (Rule 8 test 3 — pure test-quality nit; the assertion's failure-mode is already self-diagnostic via the LlmCall count assertion). Future-work: replace `Do tCfg.%Save()` with `Do $$$AssertStatusOK(tCfg.%Save(), "test setup save succeeded")` on the next `AgentLoopGuardsTest` touch.

---

## Deferred from: code review of story-12-6-chat-history-tile-replay (2026-05-08)

- **LOW — Inline-comment drift in `Catch` block between MessageViewer.cls and VisualTrace.cls `FlattenTurnsForBootstrap` flatteners.** [`src/SessionAgent/EnsPortal/MessageViewer.cls:481` vs. `src/SessionAgent/EnsPortal/VisualTrace.cls:545-548`] AC-4 mandates "byte-for-byte identical (modulo doc-comment differences if any)". The two methods' executable code is byte-identical (verified via `diff` of the method bodies — same statements, same operators, same control flow). However, the `Catch ex { ... }` block carries a 1-line inline comment in MessageViewer (`; Swallow — fall through to the empty-array degradation.`) and a 3-line inline comment in VisualTrace (`; Swallow — fall through to the empty-array degradation. The` / `; panel will render the welcome message on a corrupt history,` / `; which is the safest UX (operator can still chat).`). The asymmetry is pre-existing — VisualTrace had the longer comment before Story 12.6, and the dev correctly preserved each class's existing `Catch`-comment style when applying the Story 12.6 logic delta. **No current predicted-bug shape** — comments are not executed code; the executable-code-level verbatim-copy invariant is satisfied. Defer rationale (Rule 8 test 3 — pure cosmetic with no predicted-bug shape; comment drift was pre-existing, not introduced by Story 12.6). Future-work: harmonize the inline `Catch` comment to the longer 3-line VisualTrace form on the next dual-flattener touch (or extract `FlattenTurnsForBootstrap` into a shared helper class to eliminate the verbatim-copy invariant entirely — proposed in Epic 11 but de-scoped per `deferred-work.md` Story 10.0 Carry-Forward Item C).
- **LOW — Screenshots 1 and 3 are byte-identical (MD5 match: `efd3db3c56598bb7a2f1c4a2e716f8ee`).** [`_bmad-output/implementation-artifacts/12-6-screenshot-tile-replay-{1,3}.png`] Per Rule 12 layout-correctness evidence, the dev's intent was to capture (1) initial render with N tiles, (2) post-click VisualTrace, (3) post-Back chat panel with N tiles preserved. Screenshots 1 and 3 having identical MD5 is technically *the strongest possible evidence* that the BUG-07 fix achieves perfect tile-state preservation across navigation (no drift in any rendered pixel including timestamps, scroll position, tile order). However, the byte-identity could equally indicate the dev re-saved screenshot-1 as screenshot-3 rather than capturing a fresh post-Back screenshot. **No current predicted-bug shape** — corroborating evidence in Completion Notes lines 187-191 includes a verbatim independent DOM-probe extraction (`tileCountAfterBack: 3` matching pre-click; `tiles (verbatim): [{sessionId:79369, tcIndex:0}, {sessionId:79362, tcIndex:0}, {sessionId:79355, tcIndex:0}]`) plus the URL-path probe (`urlPath: /csp/hscustom/SessionAgent.EnsPortal.MessageViewer.zen`); the BUG-07 fix is independently verifiable via the DOM-probe trace. Defer rationale (Rule 8 test 3 — cosmetic evidence-form gap; corroborating DOM-probe evidence is independently sufficient). Future-work: on next post-navigation walkthrough, vary the screenshot capture timing (e.g., scroll position, page state) so the post-action screenshot is structurally distinguishable from the pre-action one, OR add a textual reference in the page (timestamp, session-counter) that visibly varies.

---

## Deferred from: code review of story-12-5-search-results-drive-messageviewer-table (2026-05-08)

- **LOW — `--sa-button-fg` CSS custom property referenced in `.sa-load-into-table-btn` rule but never declared anywhere in `EmitStyle()`.** [`src/SessionAgent/UI/ChatPanel.cls:287`] The new rule uses `color: var(--sa-button-fg, #ffffff);` but `--sa-button-fg` is not listed alongside `--sa-accent-color` / `--sa-citation-chip-text` / `--sa-muted-fg` in the `:root` declaration block (lines 117-167). The fallback `#ffffff` is always applied, so the rule renders correctly today. The other token references in this same diff (`--sa-accent-color`, `--sa-citation-chip-text`, `--sa-from-search-stripe-bg`) ARE declared. **No current predicted-bug shape** — the var-indirection is dead but the rendered color is correct. Defer rationale (Rule 8 test 3 — pure cosmetic; the dead var indirection has no behavioral consequence and there's no design-token authority that publishes `--sa-button-fg` for the Story 12.5 button to inherit). Future-work: either declare `--sa-button-fg` in `EmitStyle()` alongside the other tokens with a published default (e.g., `var(--zen-portal-button-text, #ffffff)`), OR drop the var indirection entirely and use `color: #ffffff;` directly to match the convention of literal-fallback-only used in the `.sa-from-search-stripe` rule pair (lines 307-311 do not invent fresh `--sa-stripe-fg` tokens for surface-specific text colors).
...
## Deferred from: code review of 14-1-knowledge-corpus-get-query-knowledge-tool (2026-06-11)

- **LOW — `Seed()`/`UpsertArticle` exists-then-act TOCTOU under concurrent invocation.** [`src/SessionAgent/Knowledge/SeedContent.cls:112-121` (UpsertArticle `SlugIdxExists` → `%New`/`%Save`)] Two processes calling `Seed()` simultaneously could both see `SlugIdxExists(slug)=0` for the same slug; the loser's `%Save` then fails on the unique `SlugIdx` and `Seed()` returns an error from a method documented as "re-running is safe" (NFR-R5). **No concurrent invocation path exists in supported flows** — the Installer runs `Seed()` once per (serial) install, and the %UnitTest runner executes test classes serially in one process (`OnBeforeOneTest` re-seeds, but never concurrently). A retry immediately converges because the upsert is idempotent. Defer rationale (Rule 8 test 3 — no predicted-bug shape under any supported execution path; the failure requires an unsupported parallel-install/parallel-test scenario, and even then fails loudly with a clear unique-index status rather than corrupting data). Future-work: on next `SeedContent` touch, either wrap the `Seed()` loop in `LOCK +^SessionAgent.KnowledgeSeedLock:5 ... LOCK -`, or treat a unique-index `%Save` failure in `UpsertArticle` as retry-via-`SlugIdxOpen`. Same shape exists in the cited `SeedVocabulary` precedent (pre-existing, out of this story's diff).

---

## Deferred from: code review of 14-2-schema-discovery-tools (2026-06-11)

- **MEDIUM — Post-`%Next()`-loop fetch-fault gate missing in PRE-EXISTING tool classes (silent-partial-rows shape).** [all `While tRS.%Next() { ... }` loops in `src/SessionAgent/Tool/Inspection/*.cls` + `src/SessionAgent/Tool/Search/*.cls` shipped before Story 14.2] `Tool.Base.EnsureIsErrorOnExecuteFailure` is invoked immediately after `%Execute` (the Story 14.0 SQLCODE sweep), but a cursor that errors MID-FETCH makes `%Next()` return 0 with a negative `%SQLCODE` — the loop ends quietly and partial rows ship inside a `render_strategy:"ok"` success envelope as if complete. Story 14.2 code review added the post-loop gate (re-calling `EnsureIsErrorOnExecuteFailure` after each fetch loop — `%SQLCODE=100` done-state passes, negative fails) to all 9 cursor sites in the 3 NEW discovery tools; the ~29 pre-existing tool classes retain the gap. Predicted-bug shape exists (Rule 8 would say fix-now) but the sweep is outside this story's diff and touches every tool class — deferred under Rule 8 test 1 (genuine future-story scope) with a named successor: **the next Story 14.x/15.x tool-sweep story (the Story 14.0 closeout-sweep pattern)** MUST grep `While t\w*RS\w*\.%Next\(\)` across `src/SessionAgent/Tool/` and add the post-loop gate per the 14.2 reference implementation (e.g., `DescribeMessageClass.AppendColumns` tail).

- **LOW — `$$$NULLOREF`-as-empty-string idiom on `%Get` reads of string args.** [`DescribeMessageClass.cls` (`class_name`), `DiscoverTables.cls` (`name_fragment`)] `Set:tVal=$$$NULLOREF tVal = ""` works because `$$$NULLOREF` expands to `""`, but an object/array-valued arg returns an OREF that is NOT `""` and flows onward. Code review type-gated the NUMERIC/BOOLEAN arg reads (`hours`, `max_results`, `include_system` — where `\ 1` coerced an OREF string form to an arbitrary small integer); the remaining string paths are safe today because the layer-2 `$Match` regex rejects an OREF's `"n@%Library.DynamicObject"` string form with a structured validation envelope. No predicted-bug shape (Rule 8 test 3). Future-work: on next tool-arg-parsing touch, normalize all arg extraction to `%GetTypeOf`-gated reads for uniformity.

- **LOW — `has_additional_info` flag name conflates property presence with child-table projection.** [`src/SessionAgent/Tool/Inspection/DescribeMessageClass.cls` `AppendCollections`] The flag is true only when the `AdditionalInfo` array property exists AND its child table is projected; a projection-suppressed AdditionalInfo property reports `false`. The census is only meaningful when projected, and stock `Ens.MessageBody`/HS hierarchies always project (Task 0 probe), so the conservative answer is the useful one — but the name overstates. No predicted-bug shape (Rule 8 test 3 — the per-property `collection_properties` entries carry the ground truth either way). Future-work: rename to `has_projected_additional_info` (envelope-shape change — coordinate with any prompt-card text in Story 14.5) or document in the tool Description.

- **LOW — `SchemaDiscoveryToolTest.cls` at 536 physical lines exceeds the ~500-line test-class guideline by ~7%.** [`src/SessionAgent/Test/SchemaDiscoveryToolTest.cls`] Review fixes (fixture Try/Catch safety) pushed 517 → 536; the two review-added test methods were placed in the new `SchemaDiscoveryEdgeTest.cls` (113 lines) instead, to bound growth. No bug shape (Rule 8 test 3). Future-work: on next touch, move the 4 discover_tables QA-gap methods into SchemaDiscoveryEdgeTest to bring both classes under 500.

---

## Deferred from: code review of 14-3-execute-readonly-sql-tool-query-base-invariant-test (2026-06-11)

- **LOW — `Audit.ToolCall.ResultSetSize` denormalized column never populated by the Dispatch path (family-wide, pre-existing).** [`src/SessionAgent/Tool/Registry.cls` `Dispatch` → `SessionAgent.Audit.Emit.LogToolCall(..., "", "", "")`] Story 14.3 AC-1 cites a "Search.Base ResultSetSize precedent" for audit enrichment, but empirically NO `SessionAgent_Audit.ToolCall` row in sample production has `ResultSetSize > 0` — `Dispatch` always passes `""` for `pResultSetSize`, so the denormalized column is dead for every one of the 33 tools (Search and Query alike). **FR60 is NOT impacted** — the meaningful enrichment (`row_count`/`truncated`/`elapsed_ms`) IS captured inside the persisted `ResultJson` envelope's `structuredContent` (verified verbatim in the live `execute_readonly_sql` audit row: `row_count:3, truncated:false, elapsed_ms:2`). The dead column is a reporting-convenience denormalization, not the audit-completeness contract. **No predicted-bug shape for this story** — the data exists in `ResultJson`. Defer rationale (Rule 8 test 1 — genuine cross-cutting scope: wiring `ResultSetSize` would require `Dispatch` to parse `structuredContent.row_count` out of every tool's envelope, a change touching the shared dispatch path for all 33 tools, well beyond 14.3's surface). Future-work / named successor: the next audit-enrichment or reporting story (or an Epic 15 observability story) should either (a) have `Dispatch` derive `ResultSetSize` from `pResult.structuredContent.row_count` when present, or (b) drop the column and document `ResultJson` as the single source of result-size truth.

- **LOW — audit args beyond 32768 chars still drop silently (swallowed-status Dispatch path).** [`src/SessionAgent/Tool/Registry.cls` `Dispatch` ignores the `%Status` returned by `SessionAgent.Audit.Emit.LogToolCall`] Story 14.3 review fix cr-14-3-C bumped `ToolCall.ArgsJson` MAXLEN 8192 → 32768, closing the realistic gap (a 15 KB SQL now audits). But a pathological > 32 KB SQL would still overflow `ArgsJson`, `%Save` would fail, and `Dispatch` swallows `LogToolCall`'s error `%Status` — so the audit row would silently not persist (NFR-S4 100%-completeness gap for that pathological input). The deeper fix is to make `Dispatch` surface (or at least not swallow) an audit-write failure, OR back `ArgsJson` with a stream when the redacted args exceed the inline ceiling — both are cross-cutting changes to the shared audit-emit path affecting all 33 tools, not just `execute_readonly_sql`. **No realistic predicted-bug shape** — LLM-authored analytical SQL is virtually never > 32 KB; the 32768 ceiling covers the entire realistic range. Defer rationale (Rule 8 test 1 — genuine cross-cutting scope: shared `Dispatch` + `Audit.Emit` robustness, beyond 14.3). Future-work / named successor: the next audit-robustness or observability story should either make `LogToolCall` failures non-silent in `Dispatch` (e.g., a fallback truncated-args row with a `args_truncated` marker) or move `ArgsJson` to a stream property.

- **DISMISSED (accepted residual, not deferred) — source-introspection invariant test cannot catch fully-dynamic dispatch.** `ReadOnlySqlInvariantTest.CheckQueryToolConformance` greps method source for forbidden tokens (`%PREPARE`/`%EXECUTE`/`&SQL(`/`%SQL.STATEMENT` after cr-14-3-D). A determined author who builds BOTH the class name AND the method name dynamically (`$ClassMethod(varHoldingClassName, varHoldingMethodName, ...)`) could still evade every source grep. This is accepted: the invariant test guards against ACCIDENTAL non-conformance by a future Query-tool author, not against a malicious committer (who has source-commit access and could ship anything). The real read-only control is the RUNTIME stage-3 `statementType` gate for code that routes through `RunGuardedSelect`, plus the L2 dispatch-level `MutatesState=0` gate. cr-14-3-D raised the bar cheaply (a non-conforming tool now must dynamic-dispatch BOTH names, which is extraordinarily contrived). No further action — recorded for transparency only.

## Deferred from: code review of 14-4-learned-schema-notes-digest-injection (2026-06-11)

- **MEDIUM — namespace-shared schema-note digest is a persistent prompt-context channel (design-accepted residual risk; hardening deferred).** [`src/SessionAgent/Knowledge/SchemaNote.cls` keying + `SchemaNoteDigest.Build` + `AgentLoop.RunTurn` first-turn injection] Notes are keyed `(Namespace, Subject)` with no per-user dimension and are replayed verbatim (160-char snippets, ≤10 lines) into EVERY conversation's first turn in that namespace — agent-authored note content authored in one operator's conversation lands in every other operator's LLM context. This is the FR63/D4 design itself (product-owner BUILD-NOW override), not an implementation defect: all operators of a namespace already share the same data visibility, content enters only via `save_schema_note` (server-side normalization + length caps), and review fix cr-14-4-G added renderer-side subject sanitization so neither subject nor body can forge the `\n\n---\n\n` prefix-block delimiter. Defer rationale (Rule 8 test 1 — design-level hardening is future-epic product scope, not a 14.4 code fix). Documented in README §"Learned schema notes" as an accepted residual risk. Future-work / named successor: a future hardening story may add per-user scoping, operator review/approval of notes, or content screening before injection.
- **MEDIUM — no retention/decay sweep for `SessionAgent_Knowledge.SchemaNote` (unbounded row growth).** [`src/SessionAgent/Knowledge/SchemaNote.cls`] The LLM can mint a new row per novel subject forever; every other agent-owned table has a purge/decay task (`PurgeOrphanedChatHistory`, `PurgeStaleSearchChat`, `UserVocabularyDecay`) but SchemaNote ships none. Review fix cr-14-4-F removed the per-first-turn full-table scan (COUNT + TOP MAXENTRIES), so growth no longer taxes every conversation — the residual cost is storage + `get_schema_notes` paging only. Defer rationale (Rule 8 test 1 — a sweep task is a story-sized operator-observable artifact: `%SYS.Task` row with populated Description per Rule 4, schedule decision, staleness threshold product call, tests). Future-work / named successor: an Epic 14+ housekeeping story should ship a `SchemaNoteDecay`-style weekly sweep (e.g., delete rows with `VerifiedAt` older than N days, N configurable) mirroring `UserVocabularyDecay`.
- **LOW — AgentLoop empty-digest-collapse assertions remain environment-conditional.** [`src/SessionAgent/Test/AgentLoopSchemaDigestTest.cls` `TestInspectionAgentEmptyDigestCollapse` / `TestSearchAgentEmptySchemaDigestStillHasVocab`] `RunTurn` builds the digest from the REAL `$NAMESPACE`, so the empirical collapse branch only executes when the live namespace has zero notes; with operator notes present the tests fall back to weaker trailer assertions plus the source-introspection lock (review fix cr-14-4-J added the collapse-construction literal `Set tInjectedUserText = "User: " _ pUserText` to that lock). Defer rationale (Rule 8 test 3 — no predicted-bug shape: the collapse contract is deterministically locked at the digest-unit level (`SchemaNoteDigestTest` now uses a synthetic namespace) and structurally in RunTurn source; making the AgentLoop path deterministic would require a namespace-injection seam in `RunTurn`, a production-code change not justified by test convenience). Future-work: if a future story adds a CallerContext/namespace seam to `RunTurn`, flip these assertions to the deterministic form.
- **LOW — JSON-arg type-guard sweep for pre-14.4 tools.** [`src/SessionAgent/Tool/**`] Review fix cr-14-4-H added `$IsObject` guards on `%Get` reads in the two NEW tools (`save_schema_note` subject/note, `get_schema_notes` subject_fragment/max_results) so a nested-object arg cannot be coerced to an OREF string ("n@%Library.DynamicObject") and persisted/matched as garbage. Older tools' `%Get` reads were not audited in this review (pre-existing, out of the 14.4 diff). Defer rationale (Rule 8 test 3 for the old tools — providers enforce `type:"string"` schemas, no observed incident; pure hardening). Future-work / named successor: the next defensive-sweep story (11.2-style) should grep `%Get(` across `Tool/**` and apply the same guard where the value is persisted or bound into SQL.

## Deferred from: code review of 14-5-prompt-methodology-card-welcome-text-golden-question-eval (2026-06-11)

- **LOW — served-asset HTTP tests silently degrade to file-on-disk fallback (no skip/fallback marker) and hardcode instance topology + superuser credentials.** [`src/SessionAgent/Test/SearchAgentRenderTest.cls` `TestChatPanelAssetServesEpic14CapabilitySentence` + pre-existing sibling `TestChatPanelAssetServesNewBytes`] Both tests claim to lock "the SERVED asset" but on any HTTP failure (server down, auth change, `/healthshare/`-prefixed web-app path instead of the hardcoded `/csp/hscustom/`, non-`_SYSTEM`/`SYS` credentials) they silently fall back to asserting the file on disk and pass green — the HTTP-serving claim is then never exercised and nothing signals the degradation. Risk is bounded: `ChatPanelAsset.OnPage` streams that same repo file at request time, so the fallback asserts the identical source of truth minus the HTTP layer, and the Story 14.5 review patch added a positive `renderWelcomeMessage` body marker closing the false-positive (wrong-200-body) direction. Defer rationale (Rule 8 test 3 leaning test-1 — the silent-fallback shape is pre-existing from Story 10.2 and a proper fix changes both tests together: emit an explicit fallback notation in the assertion text (e.g. `$$$AssertTrue(1, "SKIPPED-HTTP: ...")`), derive the web-app path/credentials from configuration instead of literals). Suggested resolution / named successor: the next test-hygiene or multi-namespace-hardening story should refactor the shared fetch into one helper with an explicit fallback marker and config-derived URL + credentials.

## Deferred from: code review of 14-6-explain-plan-reasoning-support-stretch (2026-06-11)

No deferrals. One MEDIUM auto-resolved in-story (cr-14-6-A — Registry.Dispatch e2e EXPLAIN regression lock added to `ReadOnlySqlProofBatteryTest.TestExplainSelectAllowedAllForms`; the AC-1 live-dispatch claim was session-evidence only). Two dismissals recorded for transparency:

- **DISMISSED (Rule 8 test 3, no bug shape) — stage-3 rejection text omits the EXPLAIN SELECT allowance.** [`src/SessionAgent/Tool/Query/Base.cls` `RunGuardedSelect` stage 3] "Only a single SELECT is allowed." is shown only for writes/DDL (types ≠ 1/79), for which the advice is operationally correct; stage-1 EXPLAIN rejections carry their own precise policy text; no test couples to the string. Pure cosmetic.
- **DISMISSED (intentional policy, test-locked) — read-only EXPLAIN variants rejected by the SELECT-only allowance.** `EXPLAIN (SELECT ...)`, `EXPLAIN WITH ... SELECT`, and option forms (e.g. `EXPLAIN ALT SELECT`) are rejected — the conservative reject-inner-write default applied to ALL non-`SELECT` second tokens (fail-closed; per-build plan-only semantics are an empirical premise per the 1-vs-79 drift). Documented in the class doc + error text and policy-locked by `ReadOnlySqlAdversarialTest.TestExplainEdgeShapes`. If a future story wants `EXPLAIN WITH ... SELECT` or `EXPLAIN ALT`, it must re-prove plan-only semantics on the target build and extend the two-token policy deliberately.
