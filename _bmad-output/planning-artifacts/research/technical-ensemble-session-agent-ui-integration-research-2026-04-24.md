...
> Message Search Agent. **NOTE on package naming**: the new design uses
> `SessionAgent.*` for ALL custom classes (including portal subclasses);
> references to `Custom.EnsSession.*` and `Custom.EnsPortal.*` in this
> doc reflect the frozen 2026-04-24 design.
...
> **⚠️ PARTIALLY SUPERSEDED (2026-05-01):** Phase 1 (terminal REPL via `%AI.Shell.Console`) is dropped from v1 — `%AI.Shell.*` does not exist on IRIS 2024.1. The new doc Topic 7 documents the deferral rationale and pre/cons for v1.5 reconsideration once a CSP-free agent surface is built. The Phase 2 + Phase 3 hierarchy maps below are unchanged and authoritative.
...
> **⚠️ SUPERSEDED (2026-05-01):** `%AI.Shell.Console.Run()` and `%AI.Shell.ConsoleAgent` are AI-Hub-only and absent on 2024.1. Phase 1 is dropped from v1.
...
> **⚠️ SUPERSEDED:** `%AI.Policy.ConsoleAuth` is AI-Hub-only. Read-only enforcement now uses the three-layer pattern in the new doc §"Three-Layer Read-Only Enforcement Pattern" (no console-auth dependency).
...
> **⚠️ SUPERSEDED (2026-05-01) — entire section.** `%AI.Agent.Session` does not exist on IRIS 2024.1. The cross-HTTP-request persistence pattern is correct in spirit (CSP-session pointer to a `%Persistent` row) but the row is now `SessionAgent.Chat.History` keyed on `(agentName, sessionKey, portalUser)` — not `%AI.Agent.Session` — and is **lifecycle-coupled to `Ens.MessageHeader.Purge()`** (Topic 10 in new doc, Option B chosen). The `Reconstitute()` / `%agent`-token-doesn't-survive-process-boundaries concern is moot: the new design has no Rust-side state. **What is preserved by reference**: the `%session.Data("EnsAgent", ...)` CSP-pointer pattern, the "compare cached Ens session ID to current page sessionId; reset if mismatch" guard, the row-lock concurrency model. See new doc §"CSP/Zen ↔ Agent-Loop Integration Path" — `%OpenId(id, 4)` row lock is mandatory.
...
> **🔄 PARTIALLY SUPERSEDED:** The DOM skeleton and the client-side wiring are reused as the basis for both `SessionAgent.EnsPortal.VisualTrace` and `SessionAgent.EnsPortal.MessageViewer` chat panels. Two concrete updates per the new doc: (1) markdown is rendered **client-side** via vendored `marked ≥ 18.0.2` + `Prism.js` + `DOMPurify` (loaded from `/csp/static/iris-session-agent/`, not CDN), not server-side via `%AI.System.RenderMarkdown` (which doesn't exist on 2024.1); (2) the `chatSend()` ZenMethod call is the same shape but lands in `SessionAgent.Agent.AgentLoop.RunTurn` (CSP-coupled) → tools (CSP-free / MCP-portable). See new doc §"Markdown→HTML rendering in CSP/Zen" and §"CSP/Zen ↔ Agent-Loop Integration Path".
...
> **⚠️ SUPERSEDED (2026-05-01) — entire section. Phase 1 dropped from v1.** The terminal REPL relied on `%AI.Shell.Console.Run()` and `%AI.Utils.SettingStore.Expand("@{wallet.*}")`, neither of which exists on IRIS 2024.1. The new doc Topic 7 documents the deferral rationale: building a CSP-free terminal REPL on top of `%Net.HttpRequest` would require duplicating much of the agent-loop state management without leveraging shared CSP-session storage; deferred to v1.5 once `SessionAgent.Agent.AgentLoop` is mature. The wallet-based API-key fetch is replaced by `$SYSTEM.Util.GetEnviron` or `Ens.Config.Credentials` lookup at `SessionAgent.LLM.OpenAIProvider.%OnNew()` time.
...
> **🔄 PARTIALLY SUPERSEDED (2026-05-01):** **The XData allTabs 4-tab override pattern, the DrawChatUI server-render skeleton, the GetHyperEventResources override, the SendChatMessage ZenMethod shape, the ResetChat ZenMethod, the SessionIdMatches guard, the chatSend/chatHandleKey/chatReset ClientMethods, and the Known Traps table (§8.6) are all preserved verbatim** — they are pure Zen/CSP framework patterns, no AI Hub dependency. **What is superseded**: (1) `Custom.EnsSession.Agent extends %AI.Agent` is replaced by a `SessionAgent.Agent.AgentLoop` invocation; (2) `agent.CreateSession()` / `%AI.Agent.Session.Load()` are replaced by `SessionAgent.Chat.History.LoadOrCreate(agentName, irisSessionId, portalUser)`; (3) `agent.Chat()` / `response.Content` / `MarkdownToHtml` server-side stub are replaced by `AgentLoop.RunTurn(...)` returning a markdown string that the **client** renders via `marked` + `Prism` + `DOMPurify`; (4) the `Custom.EnsSession.Agent` declarative class is gone; system prompt lives in `SessionAgent.Config.Agent` `%Persistent` rows. The new doc §"CSP/Zen ↔ Agent-Loop Integration Path" shows the substitute hyperevent → AgentLoop dispatch path verbatim. **Also note**: in the v1 design, the portal subclass is `SessionAgent.EnsPortal.VisualTrace` (not `Custom.EnsPortal.VisualTrace` as referenced below) per the saved package-naming memory.
...
> **🔄 EXTENDED (2026-05-01), NOT SUPERSEDED:** The one-line `showTrace()` ClientMethod override is unchanged. **The new doc adds a chat panel to this same `SessionAgent.EnsPortal.MessageViewer` page** to host the **Message Search Agent** (a sibling agent with a different tool set — search-by-X over `Ens.MessageHeader` rather than inspect-this-session). The XData allTabs / DrawChatUI / SendChatMessage pattern from Section 8 applies here too; both portal subclasses share the chat-panel infrastructure. See new doc §"Two-Agent Infrastructure Sharing" and the sibling Message Search Agent research doc.
...
> **🔄 PARTIALLY SUPERSEDED:** The `Custom.EnsSession.AuditLog` `%Persistent` class shape is the right idea but is replaced by **two** audit classes per the new doc §"Audit Logging Cross-Cut Pattern": `SessionAgent.Audit.LlmCall` (1 row per provider HTTP round-trip, captures all four Anthropic usage fields + OpenAI/Gemini equivalents) and `SessionAgent.Audit.ToolCall` (1 row per tool dispatch, captures duration + isError + redacted args). Both foreign-key to `SessionAgent.Chat.History`. Error UX patterns (§10.2) and `keepAlive` (§10.3) carry over unchanged.
...
> **⚠️ SUPERSEDED:** The hackathon-day code order targets the AI-Hub build path. The new doc §"Implementation Approaches and Technology Adoption" replaces this with an 11-epic IPM-installable sprint sequence and a `module.xml` skeleton. Wallet entry is replaced by env-var or `Ens.Config.Credentials`. Phase 1 is dropped (Hour 1 of §11.2 is obsolete). **What is preserved**: the package-mapping deployment recipe and the portal-resource expectations (`%Ens_MessageTrace:USE`, `%Ens_MessageHeader:USE`).
...
> **⚠️ SUPERSEDED:** The 4-hour hackathon framing is no longer current — v1 is now an 11-epic IPM-packaged build, not a hackathon. The class-name list, the agent-state-persistence one-liner (using `%AI.Agent.Session.Load`), and the "wallet key" gotcha are all AI-Hub-bound. Use the new doc §"Implementation Approaches and Technology Adoption" sprint sequencing instead. **What is preserved**: the gotchas about package-mapping (#1) and `[ ZenMethod ]` decorator (#5) carry over to any pure-ObjectScript build.
...
> **⚠️ SUPERSEDED (2026-05-01) — partial.** The §13.1 Decisions table conflates two categories. **Preserved decisions**: subclassing portal classes, deployment topology (HSCUSTOM + package-mapping), CSP-session inheritance via bookmark URL, chat UI tech (server-render skeleton + client JS + ZenMethod), context-awareness (selectedMessageId enrichment), Phase 3 handoff (1-line showTrace override). **Superseded decisions**: agent state via `%AI.Agent.Session` (replaced by `SessionAgent.Chat.History`), tool-schema state ("non-issue" relied on `ToolManager`-per-Chat() — now `SessionAgent.Tool.Registry.ListTools()` enumerates per turn), Phase 1 terminal (dropped from v1), audit logging (split into `LlmCall` + `ToolCall`). §13.4 Residual Risks #1 (EAP API churn) is moot — there is no EAP dependency. Use the new doc §"Research Synthesis & Executive Summary" as the current authoritative summary for the v1 build.
